...
<?php else :
	...
	<?php if ( ! $stats['zip_ready'] ) : ?>
		<div class="velox-alert velox-alert--warn"><?php printf( esc_html__( 'PHP-zip isn\'t available on this server, so %1$s backups can\'t be created or restored — database backups still work. Ask your host to enable the %2$s extension.', 'velox' ), '<strong>' . esc_html__( 'file', 'velox' ) . '</strong>', '<code>' . esc_html__( 'php-zip', 'velox' ) . '</code>' ); ?></div>
	<?php endif; ?>

	<div class="vbk-stats">
		<div class="vbk-stat"><span class="vbk-stat-n"><?php echo (int) $stats['count']; ?></span><span class="vbk-stat-l"><?php esc_html_e('Backups', 'velox'); ?></span></div>
		<div class="vbk-stat"><span class="vbk-stat-n"><?php echo esc_html( velox_backup_size( $stats['total_size'] ) ); ?></span><span class="vbk-stat-l"><?php esc_html_e('Total size', 'velox'); ?></span></div>
		<div class="vbk-stat"><span class="vbk-stat-n" style="font-size:15px;"><?php echo $stats['newest'] ? esc_html( date_i18n( 'M j, H:i', strtotime( $stats['newest'] ) ) ) : '—'; ?></span><span class="vbk-stat-l"><?php esc_html_e('Newest', 'velox'); ?></span></div>
		<div class="vbk-stat"><span class="vbk-stat-n"><?php echo (int) $stats['restores']; ?></span><span class="vbk-stat-l"><?php esc_html_e('Restores run', 'velox'); ?></span></div>
	</div>

	<!-- Create + import row -->
	<div class="vbk-action-grid">
		<div class="velox-panel velox-tool-form">
			<h3 class="velox-panel-title"><?php esc_html_e('Create a backup', 'velox'); ?></h3>
			<p class="velox-hint"><?php esc_html_e('Choose what to include. Large sites can take a minute — a progress box will keep you posted.', 'velox'); ?></p>
			<div class="vbk-create-row">
				<div class="vbk-seg" id="vbk-what-seg">
					<button type="button" class="vbk-seg-btn is-active" data-what="both"<?php echo $stats['zip_ready'] ? '' : ' disabled'; ?>><?php esc_html_e('Export all', 'velox'); ?></button>
					<button type="button" class="vbk-seg-btn" data-what="db"><?php esc_html_e('Export DB', 'velox'); ?></button>
					<button type="button" class="vbk-seg-btn" data-what="files"<?php echo $stats['zip_ready'] ? '' : ' disabled'; ?>><?php esc_html_e('Export files', 'velox'); ?></button>
				</div>
				<button class="velox-btn velox-btn--primary" id="vbk-create"><?php esc_html_e('Create backup', 'velox'); ?></button>
			</div>
		</div>
	...
	<!-- Backups list -->
	<div class="velox-section-title"><?php esc_html_e('Your backups', 'velox'); ?></div>
	<div class="velox-panel velox-panel--flush">
		<div id="vbk-list">
			<?php if ( empty( $backups ) ) : ?>
				<p class="velox-hint" style="padding:22px;"><?php esc_html_e('No backups yet. Create one above, import one, or set up a schedule below.', 'velox'); ?></p>
			<?php else : ?>
				<table class="vbk-table">
					<thead><tr><th><?php esc_html_e('Backup', 'velox'); ?></th><th><?php esc_html_e('Contents', 'velox'); ?></th><th><?php esc_html_e('Size', 'velox'); ?></th><th><?php esc_html_e('When', 'velox'); ?></th><th class="vbk-th-act"></th></tr></thead>
					<tbody>
						<?php foreach ( $backups as $b ) :
							...
							<tr class="vbk-row" data-id="<?php echo esc_attr( $b['id'] ); ?>" data-hasdb="<?php echo $has_db ? '1' : '0'; ?>" data-haszip="<?php echo $has_zip ? '1' : '0'; ?>">
								<td>
									...
									<?php if ( $is_safety ) : ?><span class="vbk-tag vbk-tag--safety"><?php esc_html_e('safety', 'velox'); ?></span><?php endif; ?>
									<?php if ( $is_import ) : ?><span class="vbk-tag vbk-tag--import"><?php esc_html_e('imported', 'velox'); ?></span><?php endif; ?>
								...
								<td class="vbk-act">
									...
									<?php endif; ?>
									<button class="velox-btn velox-btn--ghost velox-btn--sm vbk-restore"><?php esc_html_e('Restore', 'velox'); ?></button>
									<button class="velox-btn velox-btn--ghost velox-btn--sm vbk-delete"><?php esc_html_e('Delete', 'velox'); ?></button>
								</td>
							</tr>
						<?php endforeach; ?>
					</tbody>
	...
	<?php if ( ! empty( $history ) ) : ?>
		<div class="velox-section-title vbk-hist-head">
			<span><?php esc_html_e('Restore history', 'velox'); ?></span>
			<button type="button" class="velox-btn velox-btn--ghost velox-btn--sm" id="vbk-hist-clear"><?php esc_html_e('Clear history', 'velox'); ?></button>
		</div>
		<div class="velox-panel velox-panel--flush">
			<table class="vbk-table vbk-hist-table">
				<thead><tr><th><?php esc_html_e('When', 'velox'); ?></th><th><?php esc_html_e('Backup', 'velox'); ?></th><th><?php esc_html_e('What', 'velox'); ?></th><th><?php esc_html_e('Took', 'velox'); ?></th><th><?php esc_html_e('Result', 'velox'); ?></th><th></th></tr></thead>
				<tbody>
					<?php foreach ( $history as $h ) : ?>
						<tr data-when="<?php echo esc_attr( $h['when'] ?? '' ); ?>">
							...
							<td>
								<?php if ( ! empty( $h['ok'] ) ) : ?>
									<span class="vbk-result vbk-result--ok"><?php esc_html_e('Success', 'velox'); ?></span>
								<?php else : ?>
									<span class="vbk-result vbk-result--fail"><?php esc_html_e('Failed', 'velox'); ?></span>
								<?php endif; ?>
	...
	<!-- Schedule -->
	<div class="velox-section-title"><?php esc_html_e('Schedule', 'velox'); ?></div>
	<div class="velox-panel velox-tool-form">
		<p class="velox-hint"><?php esc_html_e('Run backups automatically via WP-Cron. Older backups beyond your keep-count are pruned after each scheduled run.', 'velox'); ?></p>
		<div class="velox-grid-2">
			<div class="velox-field">
				<span class="velox-field-label"><?php esc_html_e('Frequency', 'velox'); ?></span>
			...
			<div class="velox-field velox-field--narrow">
				<span class="velox-field-label"><?php esc_html_e('Keep newest', 'velox'); ?></span>
		...
		<div class="velox-tool-actions">
			<button class="velox-btn velox-btn--primary" id="vbk-sched-save"><?php esc_html_e('Save schedule', 'velox'); ?></button>
			<span class="velox-hint" style="margin-left:8px;"><?php esc_html_e('WP-Cron runs on traffic — quiet sites may back up a little late.', 'velox'); ?></span>
		</div>
	</div>
	...
	<div class="vbk-modal" id="vbk-modal" hidden>
		...
		<div class="vbk-modal-card" role="dialog" aria-modal="true" aria-labelledby="vbk-modal-title">
			...
			<h3 class="vbk-modal-title" id="vbk-modal-title"><?php esc_html_e('Working…', 'velox'); ?></h3>
			<p class="vbk-modal-msg" id="vbk-modal-msg"><?php esc_html_e('Please keep this tab open.', 'velox'); ?></p>
	...
	<div class="vbk-modal" id="vbk-restore-modal" hidden>
		...
		<div class="vbk-modal-card" role="dialog" aria-modal="true" aria-labelledby="vbk-rm-title">
			<h3 class="vbk-modal-title" id="vbk-rm-title"><?php esc_html_e('Restore this backup?', 'velox'); ?></h3>
			<p class="vbk-modal-msg" id="vbk-rm-msg"><?php esc_html_e('This replaces your current site with the contents of this backup.', 'velox'); ?></p>
			<label class="velox-toggle-row" style="margin:14px 0;cursor:pointer;">
				<div class="velox-toggle-meta">
					<span class="velox-toggle-label"><?php esc_html_e('Take a safety snapshot first', 'velox'); ?></span>
					<span class="velox-toggle-desc"><?php printf( esc_html__( 'Saves your %s database (a small DB-only backup) so you can undo this restore. It\'s the small one labelled “safety”.', 'velox' ), '<em>' . esc_html__( 'current', 'velox' ) . '</em>' ); ?></span>
				</div>
			...
			<div class="vbk-modal-actions">
				<button class="velox-btn velox-btn--ghost" data-close><?php esc_html_e('Cancel', 'velox'); ?></button>
				<button class="velox-btn velox-btn--primary" id="vbk-rm-go"><?php esc_html_e('Restore now', 'velox'); ?></button>
			</div>
		</div>
	</div>
