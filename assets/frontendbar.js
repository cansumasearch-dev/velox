/* Velox frontend admin quick-panel. Admins only; mounted in wp_footer.
   Hardened against script load-order: waits for VELOX_FB + the mount node
   and for DOM-ready before building, so it can't silently bail. */
( function () {
	'use strict';
	...
	function build() {
		...
		if ( mount.getAttribute( 'data-vx-built' ) === '1' ) { return true; }
		mount.setAttribute( 'data-vx-built', '1' );
		...
		setPill( barRow, D.adminBar === 1 || D.adminBar === '1' );
		...
		setPill( maintRow, D.maint === 1 || D.maint === '1' );
		...
		if ( D.editUrl ) { wrap.appendChild( rowBtn( ICON.edit, t.edit || 'Edit this page', { href: D.editUrl } ) ); }
		if ( D.oxygen && D.oxygen.active ) {
			if ( D.oxygen.editor ) { wrap.appendChild( rowBtn( ICON.oxygen, t.oxyEdit || 'Oxygen editor', { href: D.oxygen.editor } ) ); }
		...
		if ( D.guestUrl ) { wrap.appendChild( rowBtn( ICON.eye, t.guest || 'View as visitor', { href: D.guestUrl, blank: true } ) ); }
		...
		function post( doAction, data, row, ok ) {
			...
			fetch( D.ajaxUrl, { method: 'POST', credentials: 'same-origin', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: body.toString() } )
				...
				.then( function ( j ) { if ( j && j.success ) { ok && ok( j.data ); } else { toast( t.failed || 'Failed' ); } } )
	...
	// Retry until VELOX_FB + the mount node are both present (handles any script
	// load-order: deferred/async, Oxygen, cache plugins reordering the footer).
	function boot() {
		if ( build() ) { return; }
		var tries = 0;
